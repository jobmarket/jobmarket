...
class Experiment extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('job, created_at', 'required'),
			array('user_id', 'numerical', 'integerOnly'=>true),
			array('job, position', 'length', 'max'=>64),
			array('company, description', 'length', 'max'=>127),
			array('from_date, to_date', 'safe'),
			// The following rule is used by search().
			// Please remove those attributes that should not be searched.
			array('id, job, position, company, description, from_date, to_date, user_id, created_at', 'safe', 'on'=>'search'),
		);
	}
